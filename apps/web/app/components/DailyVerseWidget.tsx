...
export function DailyVerseWidget({
  ...
  const shareData = createVerseShareData(verse.text, verse.reference, 'https://bom.study');
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 pt-4 border-t border-[var(--color-border-light)] flex gap-2">
        ...
        <button className="flex-1 px-3 py-2 bg-[var(--color-bg-tertiary)] hover:bg-[var(--color-bg-secondary)] rounded-lg text-xs font-medium transition">
          Add to Study
        </button>
      </div>
    </div>
  ...
  return (
    <div className="w-full h-full bg-gradient-to-br from-purple-600 to-blue-600 p-4 text-white">
      <div className="flex flex-col h-full justify-between">
        ...
        <p className="text-sm font-semibold opacity-90">— {verse.reference}</p>
      </div>
    </div>
  );
}
