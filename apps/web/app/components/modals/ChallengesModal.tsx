...
export function ChallengesModal({
  ...
  const [selectedTab, setSelectedTab] = useState<'recommended' | 'active' | 'available' | 'completed'>('recommended');

  if (!show) return null;

  const tabs = [
    { id: 'recommended' as const, name: 'Recommended', count: recommendedChallenges.length, icon: '⭐' },
