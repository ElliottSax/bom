...
export const WordStudyModal: React.FC<WordStudyModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center pt-16 fade-in" onClick={handleClose} role="dialog" aria-modal="true" aria-labelledby="word-study-modal-title">
      <div ref={focusTrapRef} className="bg-[var(--color-bg-primary)] rounded-2xl w-full max-w-3xl mx-4 shadow-2xl border border-[var(--color-border)] overflow-hidden max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 border-b border-[var(--color-border-light)] flex items-center justify-between">
          <h2 id="word-study-modal-title" className="text-lg font-semibold">Word Study / Concordance</h2>
          <button onClick={handleClose} className="p-2 rounded-lg hover:bg-[var(--color-bg-tertiary)]" aria-label="Close word study modal"><CloseIcon aria-hidden="true" /></button>
        </div>

        {/* Search */}
        <form onSubmit={handleSearch} className="p-4 border-b border-[var(--color-border-light)]" role="search">
          <div className="relative">
            <label htmlFor="word-study-search" className="sr-only">Search for a word in scriptures</label>
            <input
            ...
            />
            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-text-tertiary)]" aria-hidden="true"><SearchIcon /></div>
            <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 px-4 py-1.5 bg-[var(--color-accent)] text-white rounded-lg text-sm font-medium hover:opacity-90" aria-label="Search for word">
              Search
            </button>
          </div>
          <span id="word-study-description" className="sr-only">Enter at least 2 characters to search for a word across all scriptures</span>
        </form>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading && (
            <div className="p-8 text-center" role="status" aria-live="polite">
              <div className="animate-spin w-8 h-8 border-2 border-[var(--color-accent)] border-t-transparent rounded-full mx-auto mb-3" aria-hidden="true"></div>
              <p className="text-sm text-[var(--color-text-secondary)]">Searching scriptures...</p>
            </div>
          )}

          {error && (
            <div className="p-8 text-center text-red-500 text-sm" role="alert">{error}</div>
          )}
            ...
            <div>
              ...
              <div className="p-4 bg-[var(--color-bg-secondary)] border-b border-[var(--color-border-light)]" role="status" aria-live="polite">
                ...
                {result.byBook.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2" role="list" aria-label="Occurrences by book">
                    {result.byBook.slice(0, 10).map(({ book, count }) => (
                      <span key={book} className="inline-flex items-center gap-1 px-2 py-1 bg-[var(--color-bg-tertiary)] rounded-lg text-xs" role="listitem">
              ...
              {/* Occurrences list */}
              <div className="divide-y divide-[var(--color-border-light)]" role="list" aria-label="Search results">
