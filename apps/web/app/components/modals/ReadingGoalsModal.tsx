...
export const ReadingGoalsModal: React.FC<ReadingGoalsModalProps> = ({ show, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center fade-in" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="reading-goals-modal-title">
      <div ref={focusTrapRef} className="bg-[var(--color-bg-primary)] rounded-2xl w-full max-w-lg mx-4 shadow-2xl border border-[var(--color-border)] overflow-hidden max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 border-b border-[var(--color-border-light)] flex items-center justify-between">
          <h2 id="reading-goals-modal-title" className="text-lg font-semibold">Reading Goals</h2>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-[var(--color-bg-tertiary)]" aria-label="Close reading goals modal"><CloseIcon aria-hidden="true" /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {activeGoals.length > 0 ? (
            <div className="space-y-4 mb-6" role="list" aria-label="Active reading goals">
              {activeGoals.map(goal => {
                ...
                return (
                  <div key={goal.id} className="bg-[var(--color-bg-secondary)] rounded-xl p-4 border border-[var(--color-border-light)]" role="listitem">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="font-medium">
                        {goal.target} {getTypeLabel(goal.type)} / {goal.period === 'daily' ? 'day' : 'week'}
                      </h3>
                      <button onClick={() => deleteGoal(goal.id)} className="text-xs text-red-500 hover:text-red-600" aria-label={`Remove goal: ${goal.target} ${getTypeLabel(goal.type)} per ${goal.period === 'daily' ? 'day' : 'week'}`}>Remove</button>
                    </div>

                    {/* Progress bar */}
                    <div className="w-full bg-[var(--color-bg-tertiary)] rounded-full h-3 mb-2" role="progressbar" aria-valuenow={stats.percentComplete} aria-valuemin={0} aria-valuemax={100} aria-label={`Goal progress: ${stats.percentComplete}% complete`}>
                      <div
          ...
          {!showCreate && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-4" role="group" aria-label="Suggested reading goals">
          ...
          {showCreate ? (
            <div className="bg-[var(--color-bg-secondary)] rounded-xl p-4 border border-[var(--color-border-light)]">
              ...
              <form onSubmit={(e) => { e.preventDefault(); handleCreate(); }} className="space-y-3">
                <div>
                  <label id="goal-type-label" className="text-xs text-[var(--color-text-secondary)] mb-1 block">Type</label>
                ...
                <div>
                  <label id="goal-period-label" className="text-xs text-[var(--color-text-secondary)] mb-1 block">Period</label>
                  <div className="flex gap-2" role="group" aria-labelledby="goal-period-label">
                    {(['daily', 'weekly'] as GoalPeriod[]).map(p => (
                      <button type="button" key={p} onClick={() => setNewPeriod(p)}
                ...
                <div>
                  <label htmlFor="goal-target" className="text-xs text-[var(--color-text-secondary)] mb-1 block">Target</label>
                  <input id="goal-target" type="number" min={1} max={100} value={newTarget} onChange={e => setNewTarget(parseInt(e.target.value) || 1)}
                    className="w-24 bg-[var(--color-bg-tertiary)] border border-[var(--color-border)] rounded-lg px-3 py-1.5 text-sm"
                    aria-label="Target amount" />
                </div>
                <div className="flex gap-2 pt-1">
                  <button type="submit" className="px-4 py-2 bg-[var(--color-accent)] text-white rounded-lg text-sm font-medium hover:opacity-90">Create</button>
                  <button type="button" onClick={() => setShowCreate(false)} className="px-4 py-2 bg-[var(--color-bg-tertiary)] rounded-lg text-sm">Cancel</button>
                </div>
              </form>
            </div>
          ) : (
