...
export const MemorizationModal: React.FC<MemorizationModalProps> = ({ show, onClose }) => {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="memorization-modal-title"
    >
      <div
        ref={focusTrapRef}
        className="bg-[var(--color-bg-primary)] rounded-2xl w-full max-w-2xl mx-4 shadow-2xl border border-[var(--color-border)] overflow-hidden max-h-[80vh] flex flex-col"
              ...
              {verses.length > 0 ? (
                <div className="space-y-2" role="list" aria-label="Memorized verses">
                  {verses.map((v) => (
                    <div
                      key={v.id}
                      className="flex items-center justify-between p-3 bg-[var(--color-bg-secondary)] rounded-xl border border-[var(--color-border-light)]"
                      role="listitem"
                    >
                      <div className="flex-1 min-w-0 mr-3">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span
                            ...
                            style={{ backgroundColor: getLevelColor(v.level) }}
                            role="status"
                            aria-label={`Level: ${getLevelLabel(v.level)}`}
                          >
                      ...
                      <button
                        ...
                        aria-label={`Remove ${v.book} ${v.chapter}:${v.verse} from memorization`}
                      >
                        Remove
                      </button>
                    </div>
              ...
              {currentVerse ? (
                <div>
                  <div className="text-center mb-6">
                    ...
                    <span
                      className="text-xs ml-2 px-2 py-0.5 rounded-full text-white"
                      style={{ backgroundColor: getLevelColor(currentVerse.level) }}
                    >
                  ...
                  {!showAnswer ? (
                    <div>
                      <div
                        className="bg-[var(--color-bg-secondary)] rounded-xl p-6 mb-4 text-center border border-[var(--color-border-light)]"
                        role="region"
                      ...
                      >
                        ...
                        {hintLevel > 0 && (
                          <p
                            className="text-sm text-[var(--color-text-secondary)] leading-relaxed mt-4 font-mono"
                            role="status"
                            aria-live="polite"
                            aria-label="Hint"
                          >
                      ...
                      <div className="flex gap-2 justify-center mb-4">
                        {hintLevel < 2 && (
                          <button
                            ...
                            aria-label={
                              hintLevel === 0
                                ? 'Show first letter of each word as a hint'
                                : 'Show more hints with partial words'
                            }
                          >
                    ...
                    <div>
                      <div
                        className="bg-[var(--color-bg-secondary)] rounded-xl p-6 mb-6 border border-[var(--color-border-light)]"
                        role="region"
                      ...
                      <p
                        className="text-center text-sm text-[var(--color-text-secondary)] mb-4"
                        role="status"
                      >
                        How did you do?
                      </p>
