...
export const MemorizationModal: React.FC<MemorizationModalProps> = ({ show, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center fade-in" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="memorization-modal-title">
      <div ref={focusTrapRef} className="bg-[var(--color-bg-primary)] rounded-2xl w-full max-w-2xl mx-4 shadow-2xl border border-[var(--color-border)] overflow-hidden max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 border-b border-[var(--color-border-light)] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h2 id="memorization-modal-title" className="text-lg font-semibold">
          ...
          <div className="flex items-center gap-2">
            {viewMode === 'practice' && (
              <button onClick={() => { setViewMode('list'); setShowAnswer(false); setHintLevel(0); }}
                ...
                aria-label="Return to verse list">
                Back to List
              </button>
            )}
            <button onClick={onClose} className="p-2 rounded-lg hover:bg-[var(--color-bg-tertiary)]" aria-label="Close memorization modal"><CloseIcon /></button>
          </div>
        </div>
            ...
            <div className="p-4">
              ...
              {dueVerses.length > 0 && (
                <button onClick={startPractice}
                  ...
                  aria-label={`Practice ${dueVerses.length} due verse${dueVerses.length !== 1 ? 's' : ''}`}>
              ...
              {/* Verse List */}
              {verses.length > 0 ? (
                <div className="space-y-2" role="list" aria-label="Memorized verses">
                  {verses.map(v => (
                    <div key={v.id} className="flex items-center justify-between p-3 bg-[var(--color-bg-secondary)] rounded-xl border border-[var(--color-border-light)]" role="listitem">
                      <div className="flex-1 min-w-0 mr-3">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="font-medium text-sm">{v.book} {v.chapter}:{v.verse}</span>
                          <span className="text-xs px-2 py-0.5 rounded-full text-white" style={{ backgroundColor: getLevelColor(v.level) }} role="status" aria-label={`Level: ${getLevelLabel(v.level)}`}>
                      ...
                      <button onClick={() => removeVerse(v.verseId)} className="text-xs text-red-500 hover:text-red-600 shrink-0" aria-label={`Remove ${v.book} ${v.chapter}:${v.verse} from memorization`}>Remove</button>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <div className="p-6">
              {currentVerse ? (
                <div>
                  ...
                  {!showAnswer ? (
                    <div>
                      <div className="bg-[var(--color-bg-secondary)] rounded-xl p-6 mb-4 text-center border border-[var(--color-border-light)]" role="region" aria-label="Practice question">
                        <p className="text-lg font-medium mb-2">Can you recite this verse?</p>
                        {hintLevel > 0 && (
                          <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed mt-4 font-mono" role="status" aria-live="polite" aria-label="Hint">
                            {getHintText()}
                          </p>
                        )}
                      </div>
                      <div className="flex gap-2 justify-center mb-4">
                        {hintLevel < 2 && (
                          <button onClick={() => setHintLevel(prev => prev + 1)}
                            ...
                            aria-label={hintLevel === 0 ? 'Show first letter of each word as a hint' : 'Show more hints with partial words'}>
                        ...
                        <button onClick={() => setShowAnswer(true)}
                          ...
                          aria-label="Reveal the verse text">
                          Show Answer
                        </button>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="bg-[var(--color-bg-secondary)] rounded-xl p-6 mb-6 border border-[var(--color-border-light)]" role="region" aria-label="Verse text">
                        <p className="text-sm leading-relaxed">{currentVerse.text}</p>
                      </div>
                      <p className="text-center text-sm text-[var(--color-text-secondary)] mb-4" role="status">How did you do?</p>
                      <div className="flex gap-3 justify-center" role="group" aria-label="Self-assessment">
                        <button onClick={() => handleAnswer(false)}
                          className="px-6 py-3 bg-red-500 text-white rounded-xl font-medium hover:bg-red-600 transition"
                          aria-label="I need more practice with this verse">
                          Needs Work
                        </button>
                        <button onClick={() => handleAnswer(true)}
                          ...
                          aria-label="I successfully recited this verse">
                          <CheckIcon aria-hidden="true" /> Got It
                        </button>
                      </div>
                    </div>
                  )}
