...
const NoteEditorModal = lazy(() =>
  import('./components/modals/NoteEditorModal').then((m) => ({ default: m.NoteEditorModal }))
);
const StudyPlanModal = lazy(() =>
  import('./components/modals/StudyPlanModal').then((m) => ({ default: m.StudyPlanModal }))
);
const BackupModal = lazy(() =>
  import('./components/modals/BackupModal').then((m) => ({ default: m.BackupModal }))
);
const ResourcesModal = lazy(() =>
  import('./components/modals/ResourcesModal').then((m) => ({ default: m.ResourcesModal }))
);
const CoCResourcesModal = lazy(() =>
  import('./components/modals/CoCResourcesModal').then((m) => ({ default: m.CoCResourcesModal }))
);
const AboutCoCModal = lazy(() =>
  import('./components/modals/AboutCoCModal').then((m) => ({ default: m.AboutCoCModal }))
);
const CoursesModal = lazy(() =>
  import('./components/modals/CoursesModal').then((m) => ({ default: m.CoursesModal }))
);
const WordStudyModal = lazy(() =>
  import('./components/modals/WordStudyModal').then((m) => ({ default: m.WordStudyModal }))
);
const ReadingGoalsModal = lazy(() =>
  import('./components/modals/ReadingGoalsModal').then((m) => ({ default: m.ReadingGoalsModal }))
);
const MemorizationModal = lazy(() =>
  import('./components/modals/MemorizationModal').then((m) => ({ default: m.MemorizationModal }))
);
const AchievementsModal = lazy(() =>
  import('./components/modals/AchievementsModal').then((m) => ({ default: m.AchievementsModal }))
);
const StreakCelebration = lazy(() =>
  import('./components/StreakCelebration').then((m) => ({ default: m.StreakCelebration }))
);
const ChallengesModal = lazy(() =>
  import('./components/modals/ChallengesModal').then((m) => ({ default: m.ChallengesModal }))
);
...
function HomeContent() {
  ...
  const {
    ...
    isLoading,
    error: versesError,
  } = useVerses(volumeId, selectedBook, selectedChapter);
  const {
    data: searchResults = [],
    isLoading: searching,
    error: searchError,
  } = useSearch(searchQuery, volumeId, 30);
  ...
  const chaptersReadInVolume = useMemo(
    () =>
      Object.keys(readingProgress.chaptersRead).filter((key) => key.startsWith(`${volumeId}:`))
        .length,
  ...
  const achievementUserData = useMemo(() => {
    ...
    const daysActiveCount = Object.values(readingProgress.chaptersRead).reduce(
      (uniqueDays, timestamp) => {
    ...
    return {
      ...
      totalScore:
        completedCoursesCount * 100 + Object.keys(readingProgress.chaptersRead).length * 10,
    ...
    <div className="min-h-screen bg-[var(--color-bg-primary)] text-[var(--color-text-primary)] flex flex-col transition-colors duration-200">
      ...
      <ErrorBoundary>
        <Suspense fallback={null}>
          ...
          {showCoCResources && <CoCResourcesModal onClose={() => setShowCoCResources(false)} />}

          {showAboutCoC && <AboutCoCModal onClose={() => setShowAboutCoC(false)} />}

          {showCourses && <CoursesModal show={showCourses} onClose={() => setShowCourses(false)} />}
      ...
      <AchievementNotificationContainer achievements={newAchievements} onDismiss={markAsViewed} />
          ...
          {!selectedBook ? (
            ...
            <ChapterReader
              ...
              getHighlight={(verseNum) =>
                getHighlight(verseNum, currentBook?.name, selectedChapter)
              }
              setHighlightColor={(verseNum, color) =>
                setHighlightColor(verseNum, color, currentBook?.name, selectedChapter)
              }
