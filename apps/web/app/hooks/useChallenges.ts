...
export function useChallenges(currentStreak: number, chaptersReadThisMonth: number) {
  ...
  const recordReading = useCallback(
    (chaptersReadToday: number, didReadToday: boolean) => {
      const updated = activeChallenges.map((progress) => {
        ...
        const updatedProgress = updateChallengeProgress(progress, chaptersReadToday, didReadToday);
  ...
  const totalPoints = getCompletedChallenges().reduce((sum, c) => sum + c.rewardPoints, 0);

  // Get completion rate
  const completionRate =
    activeChallenges.length > 0
      ? Math.round((getCompletedChallenges().length / activeChallenges.length) * 100)
      : 0;
