...
export function useCrossReferences() {
  const getCrossReferences = useCallback(
    (editionId: string, book: string, chapter: number, verse: number): CrossReference[] => {
  ...
  const hasCrossReferences = useCallback(
    (editionId: string, book: string, chapter: number, verse: number): boolean => {
