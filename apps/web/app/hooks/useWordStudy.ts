...
export function useWordStudy() {
  ...
  const [completedStudies, setCompletedStudies] =
    useState<CompletedWordStudy[]>(loadCompletedStudies);
  ...
  const completeStudy = useCallback(
    (word: string, volumeId: VolumeId, totalOccurrences: number) => {
  ...
  const isStudyCompleted = useCallback(
    (word: string, volumeId: VolumeId): boolean => {
      const studyKey = `${word.toLowerCase()}-${volumeId}`;
