...
export function useMemorization() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(MEMORIZATION_KEY, JSON.stringify(verses));
    } catch (e) {
      log.error('Save memorization failed', e instanceof Error ? e : undefined);
    }
  }, [verses]);

  const addVerse = useCallback(
    (verse: { verseId: string; book: string; chapter: number; verse: number; text: string }) => {
      setVerses((prev) => {
  ...
  const getFirstLetterHint = useCallback((text: string): string => {
    return text
      .split(' ')
      .map((w) => w[0] + w.slice(1).replace(/[a-zA-Z]/g, '_'))
      .join(' ');
  }, []);
