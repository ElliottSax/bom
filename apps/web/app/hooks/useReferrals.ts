...
export function useReferrals() {
  const [referralCode, _setReferralCode] = useLocalStorage<string>(
    'coc-referral-code',
    generateReferralCode()
  );

  const [referrals, setReferrals] = useLocalStorage<Referral[]>('coc-referrals', []);
  ...
  const getReferralUrl = useCallback(() => {
    const baseUrl = typeof window !== 'undefined' ? window.location.origin : 'https://bom.study';
  ...
  const getStats = useCallback((): ReferralStats => {
    ...
    stats.pointsEarned = stats.joinedReferrals * 10 + stats.activeReferrals * 25;
