...
export function useReadingGoals() {
  ...
  const [history, setHistory] = useState<GoalProgress[]>(() =>
    loadFromStorage(GOAL_HISTORY_KEY, [])
  );
  ...
  useEffect(() => {
    try {
      ...
      log.error('Save goals failed', e instanceof Error ? e : undefined);
    }
  }, [goals]);
  ...
  useEffect(() => {
    try {
      ...
      log.error('Save history failed', e instanceof Error ? e : undefined);
    }
  }, [history]);
  ...
  const recordProgress = useCallback(
    (goalId: string, amount: number) => {
      ...
      setHistory((prev) => {
        ...
        return [
          ...prev,
          { goalId, date: periodKey, progress: amount, completed: amount >= goal.target },
        ];
      });
    },
    [goals]
