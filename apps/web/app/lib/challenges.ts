...
export function calculateChallengeProgress(
  ...
  const percentComplete = Math.round((progress.chaptersRead / challenge.targetChapters) * 100);

  // Calculate if user is on pace
  const expectedProgress = (daysElapsed / challenge.durationDays) * challenge.targetChapters;
