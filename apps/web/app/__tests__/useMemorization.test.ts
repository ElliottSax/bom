...
describe('useMemorization', () => {
  ...
  describe('recordReview', () => {
    ...
    it('should update review timestamps', () => {
      ...
      expect(result.current.verses[0].lastReviewedAt).toBeGreaterThanOrEqual(
        beforeReview
      );
  ...
  describe('helper functions', () => {
    ...
    it('should format next review times', () => {
      ...
      expect(result.current.formatNextReview(now + 3600000 * 2)).toContain(
        'hours'
      );
      expect(result.current.formatNextReview(now + 86400000 * 2)).toContain(
        'days'
      );
    });
  });
  ...
  describe('localStorage persistence', () => {
    it('should load verses from localStorage on mount', () => {
      ...
      expect(result.current.verses[0].verseId).toBe(
        'coc-bom-1908:I Nephi:3:7'
      );
    });
