...
describe('useWordStudy', () => {
  ...
  describe('studyWord', () => {
    it('should search for a word and return results', async () => {
      ...
      let _wordResult;
      await act(async () => {
        _wordResult = await result.current.studyWord('faith', 'coc-bom-1908');
      });
    ...
    it('should lowercase search term', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('q=faith'));
    });
    ...
    it('should encode search term for URL', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('test%20%26%20trial'));
    });
    ...
    it('should include volume ID in request', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('volume=coc-dc-1908'));
    });
    ...
    it('should limit results to 200', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('limit=200'));
    });
  });
